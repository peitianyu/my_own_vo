...
TEST(stereo_vo, test) {
  ...
  // for(uint i = 0; i < 20; ++i)
  {
    ...
    // stereo.get_q().toRotationMatrix().eulerAngles(2, 1, 0);
    LOG_TEST("seq0: ", i, " ", stereo.get_t().transpose());
    ...
    //     LOG_TEST("seq1: ", i," ", gt_t.transpose());
    ...
    {
      LOG_TEST(
          "gt: ", read_kitti_dataset.get_ground_truth_q(i).coeffs().transpose(),
